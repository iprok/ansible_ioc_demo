...
| :--- | :--- | :--- |
| [hosts.yml](file:///home/cyberbat/work/demo.ansible_demo/hosts.yml) | YAML Inventory | Defines the Ansible host groups (`proxmox_hypervisors`, `gitlab_servers`, `zabbix_servers`, `app_servers`, `monitored_nodes`), mapping guest VMs to their Proxmox host hypervisors. Contains metadata specifications such as VMID, CPU cores, RAM size, disk size, and static IP parameters. |
| [group_vars/all/vars.yml](file:///home/cyberbat/work/demo.ansible_demo/group_vars/all/vars.yml) | YAML Variables | Stores global environment configurations including the deployment user (`admin_deploy`), timezone (`Europe/Sofia`), DNS servers (`1.1.1.1`, `8.8.8.8`), and domain name (`example.com`). |
| [group_vars/proxmox.yml](file:///home/cyberbat/work/demo.ansible_demo/group_vars/proxmox.yml) | YAML Variables | Defines Proxmox API settings and credentials, and sets default provisioning values for VMs (default storage, network bridge, and OS cloud-init template). |
| [group_vars/gitlab.yml](file:///home/cyberbat/work/demo.ansible_demo/group_vars/gitlab.yml) | YAML Variables | Contains GitLab deployment details such as edition, external URLs, and runner registration parameters. |
| [group_vars/zabbix.yml](file:///home/cyberbat/work/demo.ansible_demo/group_vars/zabbix.yml) | YAML Variables | Contains configuration details for Zabbix server and agent, including the server IP address (`192.168.1.60`), agent listen port (`10050`), version, and DB backend connection details. |
| [roles/proxmox_vm/defaults/main.yml](file:///home/cyberbat/work/demo.ansible_demo/roles/proxmox_vm/defaults/main.yml) | YAML Variables | Declares default hardware sizing thresholds (1 CPU core, 1024 MB RAM, 10G disk size) and default PVE configuration settings. |
| [roles/proxmox_vm/tasks/main.yml](file:///home/cyberbat/work/demo.ansible_demo/roles/proxmox_vm/tasks/main.yml) | YAML Tasks | Defines Proxmox virtualization VM lifecycle tasks (cloning, configuring resources, cloud-init setup, and power management) with `delegate_to` pointing to the respective PVE hypervisor. |
| [roles/gitlab_server/tasks/main.yml](file:///home/cyberbat/work/demo.ansible_demo/roles/gitlab_server/tasks/main.yml) | YAML Tasks | Outlines steps to install GitLab dependencies, add repo, install package, configure via template, reconfigure system, and register runners. |
| [roles/zabbix_server/tasks/main.yml](file:///home/cyberbat/work/demo.ansible_demo/roles/zabbix_server/tasks/main.yml) | YAML Tasks | Sets up MariaDB backend, creates databases and users, imports schemas, configures daemon templates, and restarts the Zabbix services. |
| [roles/zabbix_agent/tasks/main.yml](file:///home/cyberbat/work/demo.ansible_demo/roles/zabbix_agent/tasks/main.yml) | YAML Tasks | Outlines steps to install Zabbix repository, deploy agents, configure agent settings, and register nodes via Zabbix API. |
| [site.yml](file:///home/cyberbat/work/demo.ansible_demo/site.yml) | YAML Playbook | Orchestrates the overall five-stage deployment cycle mapping plays, hosts, and role executions. |
